...
DP_Ops decode (const DP_Type data) {
  cout << "DP_TYPE" << endl;
  /* For this function, add decode cases for DP_ORR, DP_CMP, DP_ADC, DP_RSB. */ }

SP_Ops decode (const SP_Type data) {
  if (data.instr.mov.op == 2) {
    // This instruction will need some special cases added for Shang to work
    // correctly, this is a minimal implementation.
    if (opts.instrs) { 
      cout << "mov";
      if (data.instr.mov.d) {
  ...
  // Here you'll need to add the decode for the SP_ADD, and SP_CMP instructions
  else {
    if (opts.instrs) { 
      cout << "nop" << endl;
    }
...
LD_ST_Ops decode (const LD_ST_Type data) {
  ...
  // Here you'll need to add LDRBR, STRBR, LDRR, STRR
  }
  else if (data.instr.class_type.opA == LD_ST_IMM_OPA) {
    // In addition to the two already shown here, you'll need STRBI and LDRBI
